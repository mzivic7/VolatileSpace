    ...
    for body_s in range(len(mass)):
        ...
        for numL in range(len(bodies_sorted[:body])):   # for previously calculated bodies:
            ...
            if rel_pos[0]*rel_pos[0] + rel_pos[1]*rel_pos[1] < coi[pot_parent]*coi[pot_parent]:   # just ultra fast check if point is inside circle - COI
                ref = pot_parent   # this body is parent
                # loop continues until smallest parent body is found
        ref_l[body_s] = ref
        rel_pos = pos[ref] - pos[body]   # relative position
        rel_vel = vel[ref] - vel[body]   # relative velocity
...
def to_newton(mass, orb_data, gc, coi_coef):
    ...
    for body_s in bodies_sorted:
        ...
        if body:
            ...
            ea = newton_root(keplers_eq, keplers_eq_derivative, 0.0, {'Ma': ma, 'e': ecc})   # newton root for keplers equation
            ...
            y_max = rot_ellipse_by_y(x_max, a, b, pe_arg)   # calculate y
